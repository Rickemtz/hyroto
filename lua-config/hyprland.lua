...
-- ============================================================
--
-- Tabla de migración rápida (config clásica → Lua):
--
--   bind = SUPER,Q,exec,kitty              → hl.bind("SUPER + Q", hl.dsp.exec_cmd("kitty"))
--   bind = SUPER,Q,killactive              → hl.bind("SUPER + Q", hl.dsp.window.close())
--   bind = SUPER,F,fullscreen              → hl.bind("SUPER + F", hl.dsp.window.fullscreen())
--   bind = SUPER,V,togglefloating          → hl.bind("SUPER + V", hl.dsp.window.float({action="toggle"}))
--   bind = SUPER,P,pseudo                  → hl.bind("SUPER + P", hl.dsp.window.pseudo())
--   bind = SUPER,1,workspace,1             → hl.bind("SUPER + 1", hl.dsp.focus({workspace=1}))
--   bind = SUPER SHIFT,1,movetoworkspace,1 → hl.bind("SUPER + SHIFT + 1", hl.dsp.window.move({workspace=1}))
--   bind = SUPER,left,movefocus,l          → hl.bind("SUPER + left", hl.dsp.focus({direction="left"}))
--
-- Puedes dividir este archivo en módulos y cargarlos con:
--   require("myColors")
-- ============================================================
...
--
-- NOTA: flameshot se lanza solo mediante keybind (SUPER+SHIFT+S),
-- no en autostart. Firefox fue reemplazado por floorp.

hl.on("hyprland.start", function()
    hl.exec_cmd(terminal)
    hl.exec_cmd("nm-applet")
    hl.exec_cmd("waybar & hyprpaper & " .. browser)
end)
...
-- Smart gaps (descomenta si quieres quitar gaps cuando hay una sola ventana):
...
hl.bind(mainMod .. " + A", hl.dsp.window.fullscreen({ mode = 1, action = "toggle" }))
hl.bind(mainMod .. " + S", hl.dsp.window.float({ action = "toggle" }))

-- Salir de Hyprland
hl.bind(mainMod .. " + M", hl.dsp.exit())

-- Scratchpad (workspace especial)
-- NOTA: se usa SUPER+Z para el scratchpad, ya que SUPER+S está en uso para float.
...
--Cambiar el togglesplit 
hl.bind(mainMod .. " + Q", hl.dsp.layout("togglesplit"))

-- Volumen
hl.bind("CTRL + SHIFT + up",   hl.dsp.exec_cmd("wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+"), { repeating = true })
hl.bind("CTRL + SHIFT + down", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-"),      { repeating = true })
hl.bind("CTRL + SHIFT + M",    hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle"))

-- Brillo
hl.bind("CTRL + " .. mainMod .. " + ALT + left",  hl.dsp.exec_cmd("brightnessctl -e4 -n2 set 5%-"), { repeating = true })
hl.bind("CTRL + " .. mainMod .. " + ALT + right", hl.dsp.exec_cmd("brightnessctl -e4 -n2 set 5%+"), { repeating = true })
...
-- Scroll sobre workspaces con el mouse
hl.bind(mainMod .. " + mouse_down", hl.dsp.focus({ workspace = "e+1" }))
hl.bind(mainMod .. " + mouse_up",   hl.dsp.focus({ workspace = "e-1" }))

-- Arrastrar / redimensionar con mouse
...
-- Fix para arrastrar en XWayland
hl.window_rule({
...
-- Ventana flotante de hyprland-run (posición fija)
hl.window_rule({
...
-- Ejemplo de layer rule (descomenta si lo necesitas):
